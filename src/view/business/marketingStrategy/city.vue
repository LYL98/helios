...
<script>
  import {Row, Col, Button, Input, Select, Option, Pagination, Table, TableColumn, Dialog, DatePicker, MessageBox, Tag} from 'element-ui';
    ...
    methods: {
      ...
      handleDelete(ids) {
        this.$messageBox.confirm('确认删除所选商品的县域定价?', '提示', {
          ...
          let res = await Http.post(Config.api.bussinessMarketingStrategyCityDelete, {
            ids: ids
          });
          if(res.code === 0){
