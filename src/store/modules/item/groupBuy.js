...
const actions = {
  ...
  async groupBuyAdd({commit, dispatch, state}, {item, success, error}) {
    ...
    if (res.code === 0) {
      dispatch('message', {title: '提示', message: '新增团购活动成功', type: 'success'});
  ...
  async groupBuyEdit({commit, dispatch, state}, {item, success, error}) {
    ...
    dispatch('loading', {isShow: false});
    if (res.code === 0) {
      dispatch('message', {title: '提示', message: '编辑团购活动成功', type: 'success'});
      typeof success === 'function' && success()
    } else {
      dispatch('message', {title: '提示', message: res.message, type: 'error'});
      typeof error === 'function' && error()
  ...
  async groupBuyActive({commit, dispatch, state}, {data, callback}){
    ...
    dispatch('loading', {isShow: false});
    if(res.code === 0){
      dispatch('message', {title: '提示', message: '团购上架成功', type: 'success'});
  ...
  async groupBuyCancel({commit, dispatch, state}, {data, callback}){
    ...
    dispatch('loading', {isShow: false});
    if(res.code === 0){
      dispatch('message', {title: '提示', message: '团购下架成功', type: 'success'});
      typeof callback === 'function' && callback(res.data);
    }else{
      dispatch('message', {title: '提示', message: res.message, type: 'error'});
    }
  ...
  async groupBuyDelete({commit, dispatch, state}, {data, callback}){
    ...
    dispatch('loading', {isShow: false});
    if(res.code === 0){
      dispatch('message', {title: '提示', message: '团购已删除', type: 'success'});
      typeof callback === 'function' && callback(res.data);
    }else{
      dispatch('message', {title: '提示', message: res.message, type: 'error'});
    }
