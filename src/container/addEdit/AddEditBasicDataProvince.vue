<template>
  <div class="province-add-eidt">
    <el-dialog :close-on-click-modal="false" :title="`${detail.id?'修改':'新增'}省`" :visible="isShow" width="720px" :before-close="handleCancel">
...
export default {
  ...
  methods: {
    ...
    async addEditData(){
      ...
      let res = await Http.post(Config.api[detail.id ? 'basicdataProvinceEdit' : 'basicdataProvinceAdd'], detail);
      this.$loading({isShow: false});
      if(res.code === 0){
        this.$message({message: `${detail.id ? '修改' : '新增'}成功`, type: 'success'});
        this.handleCancel(); //隐藏
        //刷新数据(列表)
        let pc = this.getPageComponents('TableBasicDataProvince');
        pc.getData(pc.query);
      }else{
