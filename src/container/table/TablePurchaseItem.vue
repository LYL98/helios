  ...
  <div>
    <div @mousemove="handleTableMouseMove">
      ...
      >
        ...
        <el-table-column label="编号/商品名" min-width="260">
            ...
            <div style="display: flex; align-items: center;">
              ...
              <ul style="margin: 0 10px;" :class="isEllipsis(scope.row)">
                <li :class="isEllipsis(scope.row)">
                  <a href="javascript:void(0);" :class="`title ${isEllipsis(scope.row)}`" @click.prevent="handleShowDetail(scope.row)">
        ...
        <el-table-column label="单价" prop="unit_price" min-width="100">
          <template slot-scope="scope">
            <div :class="isEllipsis(scope.row)">
              {{ scope.row.unit_price == 0 ? '' : '￥' }}{{ scope.row.unit_price == 0 ? '-' : returnPrice(scope.row.unit_price) }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="付款时间" min-width="90">
          <template slot-scope="scope">
            <div :class="isEllipsis(scope.row)">
              {{ scope.row.pay_date || '-' }}
            </div>
        ...
        <el-table-column label="操作" width="100">
          <template slot-scope="scope">
            <my-table-operate
              ...
              {
                title: '修改',
                isDisplay: (auth.isAdmin || auth.PurchaseItemModify) && scope.row.status === 'init' && !auth.PurchaseItemHighAuthor,
                command: () => handleModifyItem(scope.row)
              },
              {
                title: '高权限修改',
                isDisplay: (auth.isAdmin || auth.PurchaseItemHighAuthor),
                command: () => handleHighAuthor(scope.row)
              },
              {
                title: '操作日志',
                isDisplay: (auth.isAdmin || auth.PurchaseItemApproveLog),
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      highAuthor: { type: Function, required: true },
