          ...
          <el-table-column :key="key" :label="item.label" :minWidth="item.width" v-if="item.isShow" :prop="item.key" :sortable="item.key === 'sale_num_last' || item.key === 'available_num' ? 'custom' : false">
            <div slot-scope="scope" class="my-td-item">
              ...
              <div v-else-if="item.key === 'price_sale'">
                ...
                <template v-else-if="((scope.row.sup_type === 'local_pur' && scope.row.bd_status !== 'no_bd') || (scope.row.sup_type === 'global_pur' && scope.row.available_num > 0) && (auth.isAdmin || auth.ItemPriceFix))">
